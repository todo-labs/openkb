...
`generate` needs `OPENROUTER_API_KEY` (or `GEMINI_API_KEY`). Use `--model <id>` to select an OpenRouter model and `--output <directory>` to change the generated document directory.
